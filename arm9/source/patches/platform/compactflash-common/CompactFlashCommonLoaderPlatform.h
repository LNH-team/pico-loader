...
protected:
    ...
    ///        code performing the lock/unlock sequences, and also repeat the same logic in CardLockUnlock.
    /// @return \c true if the platform requires the sequence, or \c false otherwise.
    virtual bool RequiresLocking() const = 0;

    /// @brief Generates the patch code containing the lock/unlock routines equivalent to \see CardLockUnlock
